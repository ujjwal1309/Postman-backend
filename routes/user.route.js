  ...
  try {
    ...
    if (user.length > 0) {
      ...
      bcrypt.compare(password, hashed_pass, function (err, result) {
        if (result) {
          let { token, refreshToken } = tokencreate(
            res,
            user[0]._id,
            user[0].role
          );
...
//       return res
//         .status(500)
//         .json({ message: "Error checking token in blacklist" });
//     }
//     if (reply === 1) res.status(200).send({ isBlacklisted: "true" });
//     else res.status(200).send({ isBlacklisted: "false" });
//   });
// });

function tokencreate(res, userId, role) {
  let token = jwt.sign({ userId, role }, "ssj", {
    expiresIn: "1d",
  });
  let refreshToken = jwt.sign({ userId, role }, "ssj2", {
    expiresIn: "10d",
  });
